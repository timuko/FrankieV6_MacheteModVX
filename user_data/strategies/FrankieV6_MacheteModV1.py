...
"""
class FrankieV6_MacheteModV1(IStrategy):
    ...
    sell_should_optimize_custom_stoploss = False
    ...
    protections_max_drawdown_max_allowed_drawdown = DecimalParameter(0.01, 0.20, default=0.1, load=True, space='protection', decimals=2, optimize=protections_should_optimize_max_drawdown)
    ...
    sell_custom_stoploss_threshold = DecimalParameter(-0.05, 0, default=-0.03, space='sell', optimize=sell_should_optimize_custom_stoploss)
    sell_custom_stoploss_bail_how = CategoricalParameter(['roc', 'time', 'any'], default='roc', space='sell', optimize=sell_should_optimize_custom_stoploss)
    sell_custom_stoploss_bail_roc = DecimalParameter(-0.05, -0.01, default=-0.03, space='sell', optimize=sell_should_optimize_custom_stoploss)
    sell_custom_stoploss_bail_time = IntParameter(720, 1440, default=720, space='sell', optimize=sell_should_optimize_custom_stoploss)  
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float, current_profit: float, **kwargs) -> float:
        ...
        sroc = self.get_custom_trade_info_indicator_by_key('sroc', pair, current_time)

        stoploss = 1

        if current_profit < self.sell_custom_stoploss_threshold.value:
            if self.sell_custom_stoploss_bail_how.value == 'roc' or self.sell_custom_stoploss_bail_how.value == 'any':
                if (sroc/100) <= self.sell_custom_stoploss_bail_roc.value:
                    stoploss = 0.001
            if self.sell_custom_stoploss_bail_how.value == 'time' or self.sell_custom_stoploss_bail_how.value == 'any':
                if trade_dur > self.sell_custom_stoploss_bail_time.value:
                    stoploss = 0.001
